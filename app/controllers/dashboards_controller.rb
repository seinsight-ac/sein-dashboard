class DashboardsController < ApplicationController
  ...
  def FB dbdate
    # facebook API
    @graph = Koala::Facebook::API.new(CONFIG.FB_TOKEN)
    # facebook fans lifetime
    //@fans = @graph.get_object("278666028863859/insights/page_fans?fields=values&date_preset=today").first.first.second.first["value"]
    # facebook fans adds
    @fans_adds = @graph.get_object("278666028863859/insights/page_fan_adds_unique?fields=values&date_preset=this_year")
    //@fans_adds_day_data = @fans_adds.first['values'].flat_map{ |i|i.values.first }
    //@fans_adds_week_data = @fans_adds.second['values'].flat_map{ |i|i.values.first }
    //@fans_adds_month_data = @fans_adds.third['values'].flat_map{ |i|i.values.first }

    # facebook page users
    @pageusers = @graph.get_object("278666028863859/insights/page_impressions_unique?fields=values&date_preset=this_year")
    //@page_users_day = @pageusers.first['values'].flat_map{ |i|i.values.first }
    //@page_users_week = @pageusers.second['values'].flat_map{ |i|i.values.first }
    //@page_users_month = @pageusers.third['values'].flat_map{ |i|i.values.first }


    # facebook fans retention    
    @postsusers = @graph.get_object("278666028863859/insights/page_posts_impressions_unique?fields=values&date_preset=this_year")
    //@posts_users_day = @postsusers.first['values'].flat_map{ |i|i.values.first }
    //@posts_users_week = @postsusers.second['values'].flat_map{ |i|i.values.first }
    //@posts_users_month = @postsusers.third['values'].flat_map{ |i|i.values.first }
    @enagementsusers = @graph.get_object("278666028863859/insights/page_engaged_users?fields=values&date_preset=this_year")
    //@enagements_users_day = @enagementsusers.first['values'].flat_map { |i|i.values.first }
    //@enagements_users_week = @enagementsusers.second['values'].flat_map { |i|i.values.first }
    //@enagements_users_month = @enagementsusers.third['values'].flat_map { |i|i.values.first }
    ...
    # negative users
    @negativeusers = @graph.get_object("278666028863859/insights/page_negative_feedback_unique?fields=values&date_preset=this_year")
    //@negative_users_day = @negativeusers.first['values'].flat_map{ |i|i.values.first }
    //@negative_users_week = @negativeusers.second['values'].flat_map{ |i|i.values.first }
    //@negative_users_month = @negativeusers.third['values'].flat_map{ |i|i.values.first }

    # fans losts
    @fanslosts = @graph.get_object("278666028863859/insights/page_fan_removes_unique?fields=values&date_preset=this_year")
    //@fans_losts_day = @fanslosts.first['values'].flat_map { |i|i.values.first }
    //@fans_losts_week = @fanslosts.second['values'].flat_map { |i|i.values.first }
    //@fans_losts_month = @fanslosts.second['values'].flat_map { |i|i.values.first }

    # post activity
    @postenagements = @graph.get_object("278666028863859/insights/page_post_engagements?fields=values&date_preset=this_year")
    //@post_enagements_day = @postenagements.first['values'].flat_map { |i|i.values.first }
    //@post_enagements_week = @postenagements.second['values'].flat_map { |i|i.values.first }
    //@post_enagements_month = @postenagements.third['values'].flat_map { |i|i.values.first }
    # link clicks day only
    @linkclicks = @graph.get_object("278666028863859/insights/page_consumptions_by_consumption_type?fields=values&date_preset=this_year")
    //@link_clicks_day = @linkclicks.first['values'].flat_map { |i|i.values.first }.flat_map { |i|i.fetch('link clicks') }
  end
end
